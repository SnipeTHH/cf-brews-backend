...
component {
    ...
    public struct function generate(struct params = {}) {
        ...
        // --- 1. Set defaults (bounded between 1 and 10,000 to prevent DoS) ---
        var requestedRows = structKeyExists(params, "numRows") && isNumeric(params.numRows) ? int(val(params.numRows)) : 10000;
        var numRows = min(10000, max(1, requestedRows));
