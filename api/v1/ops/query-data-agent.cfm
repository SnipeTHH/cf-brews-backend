 ...
 */
    ...
    try {
        ...
        try {
            ...
            agentId = trim(server.system.environment.VERTEX_AGENT_ID ?: "");
            ...
            apiResult = "";
            cfhttp(url=chatUrl, method="POST", result="apiResult", timeout=60) {
                cfhttpparam(type="header", name="Authorization", value="Bearer #token#");
                cfhttpparam(type="header", name="Content-Type", value="application/json");
            ...
            if (val(apiResult.statusCode) == 200) {
                ...
                response.message = "Conversational Analytics service returned an error.";
            }

        } catch (any apiError) {
            writeLog(file="cfbrews_error", text="Real Conversational Analytics API call exception: #apiError.message#");
            cfheader(statusCode=500);
            response.message = "Conversational Analytics request failed.";
        }
        ...
        cfheader(statusCode=500);
        response.message = "Conversational Analytics gateway error.";
    }
