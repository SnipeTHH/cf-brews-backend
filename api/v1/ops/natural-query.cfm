...
REQUEST: $$ || :prompt
        ...
        // Clean up SQL string (remove markdown backticks, wrapping quotes, and trailing semicolons)
        cleanSql = trim(rawGeneratedSql);
        cleanSql = reReplace(cleanSql, "```sql|```", "", "ALL");
        cleanSql = trim(cleanSql);
        cleanSql = reReplace(cleanSql, "^[""'`]+|[""'`;]+$", "", "ALL");
        cleanSql = trim(cleanSql);

        // 4. Strict Security Validation on Generated SQL
        // Enforce single-statement SELECT against only the authorized Parameterized Secure Views
        sqlWithoutAllowedViews = reReplaceNoCase(cleanSql, "\bbrews\.(secure_batches|secure_vatsensorreadings)\b", "ALLOWED_VIEW", "ALL");
        hasAllowedView = reFindNoCase("\bbrews\.(secure_batches|secure_vatsensorreadings)\b", cleanSql) > 0;
        hasForbiddenSyntax = reFindNoCase("(;|--|/\*|\*/)", cleanSql) > 0;
        hasForbiddenKeywords = reFindNoCase("\b(DROP|DELETE|UPDATE|INSERT|ALTER|TRUNCATE|CREATE|GRANT|REVOKE|COPY|EXECUTE|CALL|DO|SET|INTO|PG_|INFORMATION_SCHEMA|LO_|DBLINK)\b", cleanSql) > 0;
        hasUnauthorizedTables = reFindNoCase("\b(FROM|JOIN)\s+(?!ALLOWED_VIEW\b)[a-zA-Z0-9_\.]+", sqlWithoutAllowedViews) > 0;

        if (
            findNoCase("BLOCKED", cleanSql) > 0 ||
            left(UCase(cleanSql), 6) != "SELECT" ||
            !hasAllowedView ||
            hasForbiddenSyntax ||
            hasForbiddenKeywords ||
            hasUnauthorizedTables
        ) {
        ...
        // 5. Execute the Generated SQL inside a hardware-enforced READ ONLY transaction
        transaction {
            queryExecute("SET TRANSACTION READ ONLY", {}, { datasource: application.cfbrews_dsn });
        ...
        if (dataResult.len() > 0) {
            try {
                ...
                writeLog(file="cfbrews_error", text="QueryData Summary Failed: #ae.message#");
                aiSummary = "I successfully retrieved the data, but could not generate a summary.";
            }
        } else {
        ...
        response.message = "QueryData translator temporarily unavailable.";
    }
