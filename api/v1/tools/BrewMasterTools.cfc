component {
    ...
    public struct function checkInventory(required string recipeName, numeric volumeGallons = 1) {
        ...
        // JOIN Recipes -> RecipeIngredients -> Inventory (match exact name first, fallback to style/fuzzy match)
        var sql = "
            WITH matched_recipe AS (
                SELECT recipe_id, recipe_name
                FROM Recipes
                WHERE LOWER(recipe_name) = LOWER(:recipe)
                   OR LOWER(style) = LOWER(:recipe)
                   OR LOWER(recipe_name) LIKE '%' || LOWER(:recipe) || '%'
                   OR LOWER(:recipe) LIKE '%' || LOWER(style) || '%'
                ORDER BY
                    CASE
                        WHEN LOWER(recipe_name) = LOWER(:recipe) THEN 1
                        WHEN LOWER(style) = LOWER(:recipe) THEN 2
                        WHEN LOWER(recipe_name) LIKE '%' || LOWER(:recipe) || '%' THEN 3
                        ELSE 4
                    END,
                    recipe_id ASC
                LIMIT 1
            ...
            SELECT
                ...
                i.unit
            FROM matched_recipe mr
            INNER JOIN RecipeIngredients ri ON mr.recipe_id = ri.recipe_id
    ...
    public struct function getRecipeDetails(required string recipeName) {
        ...
        var sql = "
            SELECT r.recipe_name, r.style, r.target_abv, r.target_ph, r.ideal_min_temp, r.ideal_max_temp,
            ...
            WHERE LOWER(r.recipe_name) = LOWER(:name)
               OR LOWER(r.style) = LOWER(:name)
               OR LOWER(r.recipe_name) LIKE '%' || LOWER(:name) || '%'
               OR LOWER(:name) LIKE '%' || LOWER(r.style) || '%'
            GROUP BY r.recipe_id
            ORDER BY
                CASE
                    WHEN LOWER(r.recipe_name) = LOWER(:name) THEN 1
                    WHEN LOWER(r.style) = LOWER(:name) THEN 2
                    WHEN LOWER(r.recipe_name) LIKE '%' || LOWER(:name) || '%' THEN 3
                    ELSE 4
                END,
                r.recipe_id ASC
            LIMIT 1
