...
component {
    ...
    public boolean function onRequestStart( required string targetPage ) {
        ...
        // Block access to hidden dot-files/directories (e.g. /.git), ColdFusion Admin (/CFIDE), or direct .cfc/WSDL introspection
        if (
            reFindNoCase("(^|/)\.", cgi.script_name) ||
            left(uCase(cgi.script_name), 6) == "/CFIDE" ||
            right(uCase(cgi.script_name), 4) == ".CFC" ||
            findNoCase("wsdl", cgi.query_string) > 0
        ) {
            cfheader(statusCode=404);
            cfcontent(type="application/json");
            writeOutput('{"error":true,"message":"Not Found"}');
            abort;
        }
    ...
    public void function onError( required any exception, required string eventName ) {
        ...
        // Return generic error without exposing internal exception types or stack traces
        cfheader(statusCode=500);
        cfcontent(type="application/json");
        var errorResponse = {"error": true, "message": "An internal server error occurred."};
